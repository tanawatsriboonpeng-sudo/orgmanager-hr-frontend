...
export default function SettingsPage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {(role === 'owner' || role === 'hr') && <LeaveTypesCard />}
...
// LEAVE TYPES (HR + owner)
// ============================================================
// CRUD for the catalog of leave categories. Backend deleteType is smart:
// if there are existing requests/quotas for the type, it soft-deletes
// (is_active=false) and returns { soft:true }; otherwise hard deletes.
// We always reload after either path so the row either disappears
// (hard) or moves to the disabled section (soft).
...
function LeaveTypesCard() {
  ...
  return (
    <div className="card mb-5">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-[#111110] flex items-center gap-2">
          <IconBeach size={15} className="text-gray-400" />
          ประเภทการลา
        </h2>
        {editing !== 'new' && (
      ...
      <p className="text-xs text-gray-500 mb-3">
        ตั้งจำนวนวัน/ปี (default ของพนักงานทุกคน), วันยื่นล่วงหน้า, และบังคับแนบหลักฐาน — โควตาของพนักงานแต่ละคนปรับได้ที่หน้า "การลา"
      </p>
      ...
      {editing === 'new' && (
        <LeaveTypeForm
          onCancel={() => setEditing(null)}
          onSaved={() => { setEditing(null); setMsg({ text: 'เพิ่มประเภทแล้ว', ok: true }); load() }}
          onError={(text) => setMsg({ text, ok: false })}
        />
      )}
...
function LeaveTypeForm({ initial, onCancel, onSaved, onError }: {
  ...
  const [carryOverDays, setCarryOverDays] = useState<number>(initial?.carry_over_days ?? 0)
  ...
  return (
    <div className="rounded-md border border-[#1D9E75]/30 bg-green-50/20 p-3 space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        <div>
          <label className="label">วันยกยอด (carry-over)</label>
          <input className="input text-sm" type="number" min={0} max={365}
            value={carryOverDays} onChange={e => setCarryOverDays(parseInt(e.target.value, 10) || 0)} />
        </div>
      </div>
